...
{
    /**
    * SEGA being themselves, they didn't use standard blowfish (which operates on big endian numbers)
    * but use an implementation that uses small endian instead.
    * This was taken from Bouncycastle.NET and modified as needed.
    */
    public sealed class BlowfishEngineLittleEndian
	{
		...
		public BlowfishEngineLittleEndian()
		{
			S0 = new uint[SBOX_SK];
			S1 = new uint[SBOX_SK];
			S2 = new uint[SBOX_SK];
        ...
        */
		private void EncryptBlock(
			...
			int dstIndex)
		{
			uint xl = BitConverter.ToUInt32(src, srcIndex);
			uint xr = BitConverter.ToUInt32(src, srcIndex + 4);
        ...
        */
		...
		internal static void UInt32_To_LE(uint n, byte[] bs, int off)
		{
			...
			Array.Copy(bytes, 0, bs, off, bytes.Length);
		}
	}
}
